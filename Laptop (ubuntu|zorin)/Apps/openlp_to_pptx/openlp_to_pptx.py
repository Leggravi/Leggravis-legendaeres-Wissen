...
try:
    ...
    print(f"Fehlende Python-Abhängigkeit: {e}", file=sys.stderr)
    print("Bitte installieren: python3 -m pip install python-pptx lxml", file=sys.stderr)
    sys.exit(1)
...
def _add_layer(slide, l, t, w, h,
    ...
    effect_color: optionaler Umrissfarbe für die Ebene.
    """
    ...
    for i, line in enumerate(lines):
        ...
        run.font.bold      = bold
        if outline_pt > 0:
            _set_outline_xml(run, outline_pt, effect_color or contrast(font_hex))
...
def add_text_block(slide, l, t, w, h,
    ...
    shadow_color = effect_color or contrast(font_hex)
    outline_color = effect_color or contrast(font_hex)

    if do_shadow:
        _add_layer(slide, l+dx, t+dy, w, h,
                   lines, is_cont, font_name, pt, shadow_color,
...
def load_theme(name):
    ...
    DEF  = dict(font_name="Arial", font_size=50, font_color="#FFFFFF",
...
def normalize_tag(tag):
    ...
    if not tag:
        return ""
    low = tag.lower().replace("_", " ").replace("–", "-").replace("—", "-")
    # titel - v1 oder title - v1 -> Strophe 1
    m = re.match(r'^(?:titel|title)\s*[-:]\s*(.+)$', low)
    if m:
        low = m.group(1).strip()

    if m := re.match(r'^(?:v(?:erse)?|strophe)\s*[-:]?\s*([0-9]+)$', low):
        return f"Strophe {int(m.group(1))}"
    if m := re.match(r'^c\s*[-:]?\s*([0-9]+)?$', low):
        return f"Refrain{' ' + m.group(1) if m.group(1) else ''}"
    if m := re.match(r'^b\s*[-:]?\s*([0-9]+)?$', low):
        return "Brücke"
    if m := re.match(r'^p\s*[-:]?\s*([0-9]+)?$', low):
        return "Pre-Refrain"
    if m := re.match(r'^r\s*[-:]?\s*([0-9]+)?$', low):
        return f"Refrain{' ' + m.group(1) if m.group(1) else ''}"
    if m := re.match(r'^(?:chorus|refrain)\s*[-:]?\s*([0-9]+)?$', low):
        return f"Refrain{' ' + m.group(1) if m.group(1) else ''}"
    if re.search(r'pre.*chor', low):
        return "Pre-Refrain"
    if re.search(r'bridge', low):
        return "Brücke"
    if re.search(r'intro', low):
        return "Intro"
    if re.search(r'outro', low):
        return "Outro"
    if re.search(r'coda', low):
        return "Coda"
    if re.search(r'interlude', low):
        return "Zwischenspiel"
    if re.search(r'ending', low):
        return "Ending"
    if re.search(r'chorus|refrain', low):
        return "Refrain"

    return " ".join(word.capitalize() for word in low.split())
...
def add_footer(slide, song, verse_tag, th, show_ccli, ccli_ov,
               ...
               footer_mode="full", effect_color=None):
    tag_s = f" – {verse_tag}" if verse_tag else ""
    if footer_mode == "none":
        return

    lines = [f"{song['title']}{tag_s}"]
    if footer_mode == "full":
...
def add_title_slide(prs, song, th, show_ccli, ccli_ov,
    ...
    title_top = TEXT_T + (FOOTER_H if corner in ('tl', 'tr') else 0) + 0.2
    add_text_block(slide, TEXT_L, title_top, 11.33, 3.8,
                   lines, ic,
                   th["font_name"], pt, th["font_color"],
                   h_align=h_al, v_align=v_al,
                   bold=True, effects=effects, shadow_offset_val=shadow_offset_val,
                   effect_color=effect_color)
    add_footer(slide, song, "", th, show_ccli, ccli_ov, corner, effects, shadow_offset_val, footer_mode="full", effect_color=effect_color)
...
def add_lyrics_slides(prs, song, vtag, text, th,
                      ...
                      footer_mode="song", effect_color=None):
    """
    split_mode: 'shrink' | 'pages' | 'maxlines'
    footer_mode: 'song' | 'none'
    """
    ...
    if split_mode == 'shrink':
        ...
        add_text_block(slide, TEXT_L, text_top, W, TEXT_MAX_H,
                       ...
                       effect_color=effect_color)
        add_footer(slide, song, vtag, th, show_ccli, ccli_ov, corner, effects, shadow_offset_val, footer_mode=footer_mode, effect_color=effect_color)
        ...
        for i, (pg_lines, pg_cont) in enumerate(pages):
            ...
            add_text_block(slide, TEXT_L, text_top, W, TEXT_MAX_H,
                           ...
                           h_align=h_al, v_align=v_al, effects=effects, shadow_offset_val=shadow_offset_val,
                           effect_color=effect_color)
            add_footer(slide, song, tag, th, show_ccli, ccli_ov, corner, effects, shadow_offset_val, footer_mode=footer_mode, effect_color=effect_color)
    ...
    for song in songs:
        ...
        for vtag, txt in song["slides"]:
            add_lyrics_slides(prs, song, vtag, txt, th,
                              ...
                              footer_mode, effect_color=effect_color)
    if black_slides:
        black_slide(prs)
    prs.save(out_path)
...
def main():
    ...
    ap.add_argument("--effect-color",     default=None,
                    help="Schatten-/Outline-Farbe als Hexwert, z.B. #888888")
    ap.add_argument("--footer-mode",      default="song",
                    choices=["song","none"],
                    help="Footer auf Lyrics-Folien: song+verse oder none")
    ap.add_argument("--beamer",           action="store_true",
                    help="Weiße Schrift auf schwarzem Hintergrund (Beamer-Modus)")
    ap.add_argument("--yellow-on-black",  action="store_true",
                    help="Gelbe Schrift mit schwarzem Hintergrund und grauem Effekt")
    a = ap.parse_args()
    ...
    th     = load_theme(tname)
    if a.beamer:
        th["font_color"] = "#FFFFFF"
        th["bg_color"] = "#000000"
        th["bg_path"] = None
    if a.yellow_on_black:
        th["font_color"] = "#FFFF00"
        th["bg_color"] = "#000000"
        th["bg_path"] = None
        th["h_align"] = 0
        th["v_align"] = 0
        a.effect_color = a.effect_color or "#888888"
    ...
    create_pptx(songs, th, a.output,
                ...
                a.footer_mode, effect_color=a.effect_color)
